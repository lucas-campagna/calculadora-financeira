<script lang="ts">
	...
	let activeStudyResult = $derived($allResultsStore[$studiesStore.activeStudyId]);

	function getVal(studyId: string, month: number, field: 'payment' | 'principal' | 'interest' | 'balance'): number | null {
