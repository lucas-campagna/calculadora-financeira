...
import type { AmortizationSystem, FinancingInput, FinancingResult } from './types';
...
export function calculate(input: FinancingInput): FinancingResult {
	...
	const principal = downPayment ? input.principal - downPayment : input.principal;
	...
	const installments = calculators[system](principal, monthlyRate, termMonths, extraPayments);
	...
	return {
		...
		installments,
		effectiveRate: totalInterest > 0 ? (totalInterest / totalPrincipal) * 100 : 0
	};
}
